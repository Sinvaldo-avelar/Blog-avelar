import { revalidateExapleAction } from "@/actions/revalidate-example";
...
export default async function ExemploDynamicPage({
    ...
    const response = await fetch('https://randomuser.me/api/?results=1', {next: {
        tags: ['randomuser'],
        revalidate: 30,
    }} )
 ...
 return <main className="min-h-[600px] text-4xl fony-bold">
    <div>
        Name: {name}| Hora: {hour} | (ID: {id})
    </div>

    <form className="py-16" action={revalidateExapleAction}>
